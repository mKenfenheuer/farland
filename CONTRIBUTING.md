...
Kerberos (`-Dkerberos`, on by default where it is found) needs MIT krb5's GSS-API (`libkrb5-dev`). Its unit tests need no KDC — they cover the principal the identity is read from and what a missing keytab does — because the handshake cannot be exercised without one. To try that, run a KDC of your own: `kdb5_util create -s -r REALM`, `kadmin.local -q "ktadd -k FILE TERMSRV/$(hostname -f)"`, a user principal, `kinit`, then `farland-server --keytab FILE --service-principal TERMSRV/$(hostname -f)` and `xfreerdp3 /v:$(hostname -f) /u:USER /d:REALM /p: /sec:nla`. `krb5kdc` takes its ports from `[kdcdefaults]`, not from the realm's `kdc =` line, which is a good half hour if you do not know it.
