...
// Manual check of the headless GNOME desktop against a real GNOME Shell (not
// run as a test): starts it (or attaches with --attach), then asks for each
// layout in turn and reports the screens the desktop ends up with.
//
//   farland-gnome-headless-probe [--attach] [--keymap LAYOUT] [--hold SECONDS]
...
// gives per second, to see what the desktop does while something happens on
// its seat; --poke moves the pointer each second while it holds.
...
int main(int argc, char** argv)
{
    ...
    auto started = farland::app::start_gnome_headless(options);
    if (!started) {
    ...
    if (hold > 0) {
        ...
        // second, to watch what the desktop does while something else
        // happens on its seat; then lets go, as a client disconnecting does.
        std::printf("a client takes the desktop\n");
