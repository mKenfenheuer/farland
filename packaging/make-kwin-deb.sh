...
# fork's commits sit on: anything else would apply to the wrong KWin. KDE
# tags releases with a "v" in front of the version.
version=$(dpkg-parsechangelog -l"$tree/debian/changelog" -SVersion)
upstream=${version%%-*}
upstream=${upstream#*:}
tag=v$upstream
if ! git -C "$fork" rev-parse --verify --quiet "refs/tags/$tag" >/dev/null; then
    # A clone of one branch has no tags; the fork carries the release its
    # commits sit on.
    git -C "$fork" fetch --quiet origin "refs/tags/$tag:refs/tags/$tag" 2>/dev/null || true
fi
if ! git -C "$fork" rev-parse --verify --quiet "refs/tags/$tag" >/dev/null; then
    echo "the fork has no tag $tag: it is not the release this distribution packages" >&2
    exit 1
fi
if ! git -C "$fork" merge-base --is-ancestor "refs/tags/$tag" HEAD; then
    echo "the fork's branch is not based on $tag, which this distribution packages" >&2
    exit 1
fi

rm -rf "$work/patches"
mkdir -p "$work/patches"
git -C "$fork" format-patch --no-signature -o "$work/patches" "$tag..HEAD" >/dev/null
if [ -z "$(ls -A "$work/patches")" ]; then
    echo "the fork has no commits on top of $tag: nothing to add" >&2
    exit 1
fi
