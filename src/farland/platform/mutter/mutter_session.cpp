...
using portal::detail::MessagePtr;
...
// --- The user's session on its seat

namespace {

/// The user's graphical session: its logind object path. This process runs
/// in a unit of the user's systemd manager, which is a session of its own
/// without a seat, so the session has to be looked up through the user.
MutterResult<std::string> display_session_path(sd_bus* bus)
{
    portal::detail::BusError error;
    sd_bus_message* reply = nullptr;
    const std::string user_path = std::format("/org/freedesktop/login1/user/_{}", ::getuid());
    const int r = sd_bus_get_property(bus, "org.freedesktop.login1", user_path.c_str(), "org.freedesktop.login1.User",
                                      "Display", error.get(), &reply, "(so)");
...
MutterResult<std::string> our_session_path(sd_bus* bus)
{
...
bool session_flag(sd_bus* bus, const std::string& path, const char* name)
{
    portal::detail::BusError error;
    int value = 0;
    return sd_bus_get_property_trivial(bus, "org.freedesktop.login1", path.c_str(), "org.freedesktop.login1.Session",
                                       name, error.get(), 'b', &value) >= 0 &&
           value != 0;
}

}  // namespace

MutterResult<void> activate_user_session()
{
    sd_bus* raw = nullptr;
    ...
    portal::detail::BusError error;
    // A session without a seat (GDM's headless ones) is always drawn.
    std::string seat_path;
    std::uint32_t vt = 0;
    {
        sd_bus_message* reply = nullptr;
        const int r = sd_bus_get_property(raw, "org.freedesktop.login1", path.c_str(), "org.freedesktop.login1.Session",
                                          "Seat", error.get(), &reply, "(so)");
        const portal::detail::MessagePtr owned(reply);
        if (r < 0) {
            return portal::detail::fail_call("reading the session's seat", error.get(), r);
        }
        MessageReader reader(owned.get());
        std::string seat_id;
        if (!reader.enter('r', "so") || !reader.string(seat_id) || !reader.string(seat_path) || !reader.exit()) {
            return fail(PortalErrc::protocol, "malformed seat");
        }
        if (seat_id.empty()) {
            return {};
        }
        static_cast<void>(sd_bus_get_property_trivial(raw, "org.freedesktop.login1", path.c_str(),
                                                      "org.freedesktop.login1.Session", "VTNr", error.get(), 'u', &vt));
    }
    ...
    static_cast<void>(sd_bus_call_method(raw, "org.freedesktop.login1", path.c_str(), "org.freedesktop.login1.Session",
                                         "Activate", error.get(), nullptr, ""));
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        if (attempt == 0 && !seat_path.empty() && vt != 0) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg): sd-bus's typed call
            static_cast<void>(sd_bus_call_method(raw, "org.freedesktop.login1", seat_path.c_str(),
                                                 "org.freedesktop.login1.Seat", "SwitchTo", error.get(), nullptr, "u",
                                                 vt));
        }
    }
    return fail(PortalErrc::failed, "the session stays off its seat, so GNOME does not draw it");
}

MutterResult<void> switch_seat_to_greeter()
{
    // GDM's local display factory lives on the system bus; its policy lets
    // any user ask for a greeter, which is what GNOME's "switch user" does.
    sd_bus* raw = nullptr;
    if (const int r = sd_bus_open_system(&raw); r < 0) {
        return fail(PortalErrc::unavailable, std::format("cannot connect to the system bus: {}", std::strerror(-r)));
    }
    ...
    const int r = sd_bus_call_method(
        system_bus.get(), "org.gnome.DisplayManager", "/org/gnome/DisplayManager/LocalDisplayFactory",
        "org.gnome.DisplayManager.LocalDisplayFactory", "CreateTransientDisplay", error.get(), &reply, "");
    const portal::detail::MessagePtr owned(reply);
    if (r < 0) {
        return portal::detail::fail_call("CreateTransientDisplay", error.get(), r);
    }
    return {};
}
...
MutterResult<std::unique_ptr<SeatWatch>> SeatWatch::create()
{
    ...
    const int r = sd_bus_match_signal_async(raw, &slot, "org.freedesktop.login1", watch->session_path_.c_str(),
...
int SeatWatch::on_properties_changed(sd_bus_message* message, void* userdata, sd_bus_error* /*error*/)
{
    ...
    if (!reader.string(interface) || interface != "org.freedesktop.login1.Session") {
        return 0;
    }
    bool active = false;
    bool seen = false;
