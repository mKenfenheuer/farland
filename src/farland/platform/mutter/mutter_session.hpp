...
/// Watches the user's graphical session in logind. GNOME only draws the
/// session that is active on its seat, so a connection holds the session
/// only while it is active: when someone switches the seat away (to another
/// session or to a greeter), Mutter stops producing frames and the
/// connection has to end.
class SeatWatch {
public:
    /// Watches this process's session; fails without logind or a session.
    [[nodiscard]] static MutterResult<std::unique_ptr<SeatWatch>> create();
    ...
    [[nodiscard]] int fd() const noexcept;
    void process();
    /// The session left its seat after we started watching: something else
    /// is on the seat now, and GNOME has stopped drawing this session.
...
/// Switches the seat this session is on to a GDM greeter, as GNOME's "switch
/// user" does (org.gnome.DisplayManager.LocalDisplayFactory.CreateTransientDisplay
/// on the system bus): the seat shows the login screen, and logging in again
/// there brings the session back to the seat.
///
/// Careful: GNOME draws only the session that is active on its seat, so a
/// session switched away this way stops producing frames, and a client
/// holding it sees nothing. Attaching therefore does the opposite
/// (activate_user_session()); this is here for the policy that wants the
/// seat's screen freed, and the caller has to live with the black picture.
[[nodiscard]] MutterResult<void> switch_seat_to_greeter();

/// Brings the user's graphical session to its seat if something else is
/// there (logind Session.Activate, and the seat's VT switch where that is
/// not enough): GNOME draws only the session that is active on its seat, so
/// a session has to be active before a client can see anything of it.
/// Succeeds at once for a session without a seat (a headless one GDM
/// started), which is always drawn.
[[nodiscard]] MutterResult<void> activate_user_session();
