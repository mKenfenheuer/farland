...
- **Status of rdpei: implemented, and tried with a real touch client; a pen is still untried.**
  - Done:
...
- **Status (2026-09-19): S0, S1, S3, S4 and S5 done, and mstsc and Windows App reach a session; the GNOME route of S2 works. S5 in full: policies, the control API with polkit, self-enrolment, `farlandctl sessions`/`terminate`, Prometheus metrics, Kerberos, the systemd units, and packaging as deb (built and installed by CI), rpm, an AUR recipe and a container image.**
  - Done:
    ...
    - `farlandctl sessions` and `farlandctl terminate` (S5), tried live on Kubuntu 26.04 against the packaged daemon: a client connects, `farlandctl sessions` shows `1  farlandtest  running  test  127.0.0.1:38764  9s  4s`, `farlandctl terminate 1` answers "ending 1 session", and the next listing is empty. Two methods on the control API, `ListSessions` and `TerminateSession`. The questions arrive on the D-Bus thread and the answers live in the daemon's loop, so a `SessionView` sits between them: the loop publishes a snapshot of its registry after every turn, and a request to end a session waits there until the loop picks it up (no I/O, no clock, tested on its own). Anyone may list and end their **own** sessions; anything else needs the polkit action `org.farland.Farland1.manage-sessions` (auth_admin), and a caller without it sees only their own in the listing rather than an error. `terminate` takes a session id or `--user USER` for every session of an account, and ends them the way a policy timeout does (broker `Terminate`, EndReason `terminated`).
    ...
    - **Sessions survive a farlandd restart**, and so do the clients connected to them: a package upgrade is invisible to everyone using the machine. It works because of how the parts are already divided — the agent owns the desktop and holds the client's socket, and the network process holds the TLS connection; farlandd is only the broker, and passed the socket on without keeping a copy. Nothing had to move, only to stop being torn down:
      - farlandd writes its sessions to `sessions` in the state directory (0600: it holds a token each) and reads them back at startup, restoring each with the id it had and waiting two minutes for its agent.
      - Shutdown no longer sends Terminate to the agents. It writes the table, closes the socket and goes.
      - The agent keeps the desktop and its client when farlandd disappears, and looks for the socket every half second for two minutes; it greets the new farlandd with the token it kept, which is why the token is no longer wiped after the first greeting. An agent whose session is gone is told so (a Terminate on a hello that matches nothing) rather than retrying to its timeout.
