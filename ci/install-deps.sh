...
debian | ubuntu)
    ...
    apt-get install -y --no-install-recommends ca-certificates git meson ninja-build pkg-config g++ clang libssl-dev \
        libsystemd-dev libei-dev libeis-dev libpipewire-0.3-dev libspa-0.2-dev libgbm-dev libegl-dev libva-dev libwayland-dev libpam0g-dev libkrb5-dev \
...
fedora)
    dnf install -y git meson ninja-build pkgconf-pkg-config gcc-c++ clang openssl-devel \
        systemd-devel libei-devel pipewire-devel mesa-libgbm-devel libglvnd-devel libva-devel wayland-devel pam-devel krb5-devel \
...
arch)
    pacman -Syu --noconfirm --needed git meson ninja pkgconf gcc clang openssl \
        systemd libei pipewire mesa libglvnd libva wayland dbus python-dbus python-gobject pam libxkbcommon wayland-protocols krb5
    ;;
*)
